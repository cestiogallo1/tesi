...
def ultimi_paydex(row):
    ...
    # Ritorna sempre una lista di 10 valori (anche se ci sono meno di 10)
...
def ultimi_FIPS(row):
    ...
    if ultimo_anno is not None and not pd.isna(ultimo_anno):
        ...
        if not fips_index.empty:
            ...
            start_index = max(0, last_index - anni)  # +1 per includere l'anno corrente
...
def rimuovi_paydex_min_max(df):
    columns_to_remove = df.filter(regex='PayDexMin|PayDexMax|PayDex1$|UltimoAnnoUtile').columns
...
def main():
    ...
    # Salva i DataFrame per fare decoding successivamente
    df.to_csv('./Encoding/encoded_data.csv', index=False)
    df_original.to_csv('./Encoding/original_data.csv', index=False)
